...
export class PerspectiveCamera implements ICamera {
    ...
    constructor(width: number, height: number) {
        ...
        window['getPos'] = (): vec3 => this.position;
        window['getView'] = (): vec2 => vec2.fromValues(this.verticalAngle, this.horizontalAngle);
    }
    ...
    public onMessage(message: Message) {
        switch (message.type) {
            case MessageType.SetAng:
                this.setAng(message.data);
                break;
            case MessageType.SetPos:
                this.setPos(message.data);
                break;
